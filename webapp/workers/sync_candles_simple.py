"""
Worker simplificado para sincronizar candles históricos
"""
...
def sync_candles_simple():
    """Sincronizar candles históricos de forma simplificada"""
    start_time = datetime.now()
    logger.info("Iniciando sincronização simplificada de candles...")
    ...
    try:
        ...
        assets = postgres_manager.get_active_assets(limit=3)  # Apenas 3 para teste
        logger.info(f"Encontrados {len(assets)} ativos para sincronizar")
        
        total_candles = 0
        timeframes = ['1m', '5m']  # Apenas 2 timeframes para teste
            ...
            for timeframe in timeframes:
                try:
                    ...
                    historical_data = streamer.stream(
                        ...
                        symbol=ticker,
                        timeframe=timeframe,
                        numb_price_candles=100  # Apenas 100 candles para teste
                    )
                    
                    candles_count = 0
                    if 'ohlc' in historical_data and historical_data['ohlc']:
                        logger.info(f"    Recebidos {len(historical_data['ohlc'])} candles")
                        ...
                        for candle in historical_data['ohlc']:
                            ...
                            # Inserir diretamente sem verificar duplicatas (para teste)
                            success = postgres_manager.create_candle(candle_record)
                            if success:
                                candles_count += 1
                                total_candles += 1
                        
                        logger.info(f"    {candles_count} candles {timeframe} inseridos")
                    else:
        ...
        logger.info(f"Sincronização concluída: {total_candles} candles inseridos")
...
if __name__ == '__main__':
    print("=" * 60)
    print("SINCRONIZADOR SIMPLIFICADO DE CANDLES")
    print("=" * 60)
    
    if sync_candles_simple():
        print("\n" + "=" * 60)
