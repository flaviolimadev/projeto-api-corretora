#!/usr/bin/env python3
"""
Script de teste simples para a API de Categorias (sem emojis)
...
def test_categories_api():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   Total de simbolos populares: {data['statistics']['total_popular_symbols']}")
            ...
            for category_id, category in data['categories'].items():
                ...
                print(f"     Simbolos: {len(category['popular_symbols'])} populares")
        else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   Total de simbolos: {data['total_symbols']}")
            print(f"   Exchanges: {', '.join(data['exchanges'])}")
            print(f"   Simbolos populares:")
            for symbol in data['popular_symbols'][:5]:
                print(f"     - {symbol}")
            if len(data['popular_symbols']) > 5:
                print(f"     ... e mais {len(data['popular_symbols']) - 5} simbolos")
        else:
            print(f"ERRO: {response.status_code}")
    ...
    for category in categories_to_test:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                print(f"   {data['name']}: {elapsed:.2f}s - {data['total_symbols']} simbolos")
            else:
